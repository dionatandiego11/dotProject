...
class AcaoRepository
{
    ...
    private function normalizeRow(array $row): array
    {
        return [
            ...
            'status_saude' => $row['status_saude'] ?? null,
            ...
            'valor_executado' => isset($row['valor_executado']) ? (float) $row['valor_executado'] : null,
    ...
    private function buildWritePayload(array $data, bool $isCreate): array
    {
        ...
        $possibleValues = [
            ...
            'tipo' => isset($data['tipo']) ? trim((string) $data['tipo']) : null,
            ...
            'valor_executado' => isset($data['valor_executado']) ? (float) $data['valor_executado'] : null,
    ...
    private function createDefaultAcaoTable(): void
    {
        $sql = "CREATE TABLE IF NOT EXISTS `dotp_acoes` (
            ...
            `tipo` VARCHAR(30) NOT NULL DEFAULT 'Projeto',
            `objetivo` TEXT NULL,
            `descricao` TEXT NULL,
            `estado` VARCHAR(40) NOT NULL DEFAULT 'Planejamento',
            `status_saude` VARCHAR(20) NOT NULL DEFAULT 'em_dia',
