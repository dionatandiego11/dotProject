...
for ($x = 0; $x < $nums; $x++) {
	...
	$projects[$row['task_project']]['tasks'][] = \DotProject\Entity\Task::fromArray($row);
...
// else users would be able to arbitrarily run 'bad' functions
	if ($showEditCheckbox) {
		...
		function checkAll(project_id) {
			var f = document.forms['assFrm' + project_id];
	...
	<table width='100%' border='0' cellpadding='1' cellspacing='0'>
		<form name='task_list_options' method='POST' action='<?php echo $query_string; ?>'>
			...
			<tr>
				<?php if ($min_view) { ?>
					<td>
						<?php echo $AppUI->_('Expand'); ?>&nbsp;<a
							href="<?php echo 'index.php' . $query_string . '&amp;open_task_all=1'; ?>"><?php echo $AppUI->_('All'); ?></a>/<a
							href="<?php echo 'index.php' . $query_string . '&amp;open_one_level=1'; ?>"><?php echo $AppUI->_('One Level'); ?></a>
						:
						<?php echo $AppUI->_('Collapse'); ?>&nbsp;<a
							href="<?php echo 'index.php' . $query_string . '&amp;close_task_all=1'; ?>"><?php echo $AppUI->_('All'); ?></a>/<a
							href="<?php echo 'index.php' . $query_string . '&amp;close_one_level=1'; ?>"><?php echo $AppUI->_('One Level'); ?></a>
						&nbsp;&nbsp;
					</td>
				<?php }
				?>
				<td align='right'>
					<table>
						<tr>
							...
							<td>
								<input type="checkbox" name="show_incomplete" value="1" id="show_incomplete"
									onclick="javascript:document.task_list_options.submit();" <?php echo $showIncomplete ? 'checked="checked"' : ''; ?> />
							</td>
		...
		if ($tnums || $project_id == $p['project_id']) {
									...
									<td align="right">
										...
										<td align="center">
											...
											<select class="text" name="percentage_assignment"
												title="<?php echo ($AppUI->_('Assign with Percentage')); ?>">
												<?php
												for ($i = 0; $i <= 100; $i += 5) {
			...
			if (!empty($p['tasks']) && is_array($p['tasks'])) {
				...
				foreach ($p['tasks'] as $i => $t1) {
					...
					if (!empty($task_sort_item1)) {
						...
						if (
							!empty($t1['task_parent']) && !empty($t1['task_id'])
			...
			?>
				<tr>
					...
					<td><?php if (!empty($summaries['start_date'])) {
						$summary_date = new CDate($summaries['start_date']);
						echo $summary_date->format($df);
					} else {
						echo "--";
					} ?>
					</td>
					<td align="center"><?php echo $summaries['duration'] ?? ''; ?> 		<?php echo $AppUI->_('hours') ?? ''; ?></td>
					<td><?php if (!empty($summaries['end_date'])) {
						$summary_date = new CDate($summaries['end_date']);
						echo $summary_date->format($df);
					} else {
						echo "--";
					} ?>
...
-->
							<input type="button" class="button" value="<?php echo $AppUI->_('Gantt Chart'); ?>" onclick="javascript:window.location='index.php?m=tasks&amp;a=viewgantt&amp;project_id=<?php
							   echo $k; ?>';" />
				</form>
				</td>
				</tr>
